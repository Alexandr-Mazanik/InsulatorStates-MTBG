...
def main():
  ...
  print("===========================================")
  print("--> Moire energy bands calculation:")
  print(f"--> theta: {np.degrees(theta)} degrees")
  print(f"    cutoff: {cutoff}")
  print(f"    w = {w} meV, t = {t} meV\n")
...
def T_b():
  return w * np.array([[1, 1], [1, 1]], dtype=complex)


def T_tr():
  return w * np.array([[np.exp(-1j * phi), 1],
   [np.exp(1j * phi), np.exp(-1j * phi)]], dtype=complex)
...
def moire_brillouin_path():
  ...
  k_AC_x = np.zeros(N)
  k_AC_y = np.linspace(-k_moire, k_moire, N)
  k_CD_x = np.arange(0, k_moire * np.sqrt(3) / 2, k_moire * np.sqrt(3) / 2 / N)
  k_CD_y = - k_CD_x * np.sqrt(3) + k_moire
  k_DA_x = k_CD_x[::-1]
  k_DA_y = k_DA_x / np.sqrt(3) - k_moire

  k_brillouin_x = np.concatenate([k_AC_x, k_CD_x, k_DA_x])
  k_brillouin_y = np.concatenate([k_AC_y, k_CD_y, k_DA_y])
...
def plot_bands(bands, brillouin_path):
  ...
  high_symmetry_labels = ['A', 'B', 'C', 'D', 'A']
  ...
  ax.set_title(f"theta = {np.degrees(theta)}, cutoff = {cutoff}", fontsize=12)
  ...
  filename = f"bands{num_bands_show}_theta{int(np.degrees(theta) * 100)}_w{w}_cutoff{cutoff}.pdf"
